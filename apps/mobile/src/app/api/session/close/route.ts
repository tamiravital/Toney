...
// Vercel Pro: 300s timeout for notes generation.
// Evolution now happens in POST /api/session/feedback (triggered after user submits feedback).
 ...
 * Sets evolution_status: 'pending' — evolution runs when the user
 * submits feedback (or dismisses notes) via POST /api/session/feedback.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Load data in parallel (notes generation only) ──
    const [messagesResult, profileResult, sessionCardsResult, sessionResult, prevNotesResult, focusAreasResult, winsResult] = await Promise.all([
      ctx.supabase
    ...
    // evolution_status = 'pending' — triggered by POST /api/session/feedback
    ...
    // ── Response (immediate) ──
    // Evolution happens when user submits feedback or dismisses notes
